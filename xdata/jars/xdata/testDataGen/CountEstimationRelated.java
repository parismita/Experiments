...
public class CountEstimationRelated {
	...
	public static boolean isAssignmentSatisfiable(GenerateCVC1 cvc, QueryBlockDetails queryBlock) throws Exception{
		...
		while(itr.hasNext()){
			...
			for(int i=0;i<aggCons.size();i++)	{	
				...
				CVCText +=GenerateConstraintsForHavingClause.getMinSumMaxVariableDeclaration(cvc, queryBlock, aggCons.get(i));
	...
	public static int UpdateNoOfOutputTuples(GenerateCVC1 cvc, QueryBlockDetails queryBlock, int minTuple) throws Exception{
		QueryBlockDetails q = queryBlock;
		minTuple = Math.max(minTuple, q.getFinalCount());
		for (String table : q.getBaseRelations()) {
			table = table.replaceAll("\\d", "").toUpperCase(); //pre process - check if there is code for that
			if(cvc.noOfMaxOutputTuplesContains(table)){
				int curr = cvc.getNoOfMaxOutputTuples(table);
				cvc.putNoOfMaxOutputTuples(table, Math.max(minTuple, curr));
			}
			else{
				cvc.putNoOfMaxOutputTuples(table, minTuple);
			}
		}
		cvc.putNoOfMaxOutputTuples("JSQ"+String.valueOf(q.getLevel()), minTuple);
	...
	public static boolean estimateCountAndgetTupleAssignmentForQueryBlock(GenerateCVC1 cvc, QueryBlockDetails queryBlock) throws Exception{
		...
		if(queryBlock.getAggConstraints() != null && queryBlock.getAggConstraints().size()!=0){
			...
			if(success == false){
				...
				int a = queryBlock.getFinalCount()+1;
				if(a<3)
	...
	public static boolean getCountAndTupleAssignmentToKillExtraGroupByMutations(GenerateCVC1 cvc, QueryBlockDetails qbt) throws Exception {
		...
		if( qbt.getAggConstraints() == null || qbt.getAggConstraints().size() == 0 ){
			...
			qbt.setFinalCount(2);		// why?
			
			// qbt.setFinalCount(1); // set by sunanda
