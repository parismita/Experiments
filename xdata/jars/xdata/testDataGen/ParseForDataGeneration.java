	 ...
	 */
		public QueryStructure parseForDataGeneration(QueryStructure qs) throws Exception{
			...
			for(ConjunctQueryStructure con : qs.getConjuncts()){
				...
				Vector<Node> inClauseConds = new Vector<Node>();
				...
				for(Node n : con.getAllSubQueryConds()){
					...
					if((n.getType().equals(Node.getInNodeType()) || n.getType().equals(Node.getNotInNodeType()) )
						&& n.getRight()!=null && n.getRight().getSubQueryStructure()!=null ){
						...
						qs.aliasList.add(n.getLeft());
						qs.aliasList.add(rhsNew);


						Node sqNode=n.getRight().clone();
						sqNode.getSubQueryStructure().getConjuncts().get(0).getSelectionConds().add(inNode);
						if(sqNode.getSubQueryStructure().getAllDnfSelCond().size()==0)
						sqNode.getSubQueryStructure().getAllDnfSelCond().add(new Vector<>());
						if(sqNode.getSubQueryStructure().getDnfCond().size()==0)
						sqNode.getSubQueryStructure().getAllDnfSelCond().add(new Vector<>());

						sqNode.getSubQueryStructure().getAllDnfSelCond().get(0).add(inNode);
						sqNode.getSubQueryStructure().getDnfCond().get(0).add(inNode);
						sqNode.getSubQueryStructure().getLstSelectionConditions().add(inNode);
						...
						sqNode.setType(Node.getExistsNodeType());
						 ...
						 if(n.getRight()!=null&& n.getRight().getSubQueryStructure().getAllDnfSelCond() != null
					...
					else if((n.getType().equals(Node.getInNodeType()) || n.getType().equals(Node.getNotInNodeType()) )
						&& n.getRight()!=null ){
						inClauseConds.add(n);
						subQCondToRemove.add(n);
					}
					...
					if(n.getType().equals(Node.getExistsNodeType()) || n.getType().equals(Node.getNotExistsNodeType())){
						...
						if(n.getType().equals(Node.getExistsNodeType())){					
							...
							existsNode.setType(Node.getNotExistsNodeType());
							newSubQCond.add(existsNode);
						}
					}
				...
				if(!inClauseConds.isEmpty()) con.setInClauseConds(inClauseConds);
