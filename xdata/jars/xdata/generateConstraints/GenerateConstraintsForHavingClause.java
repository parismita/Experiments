...
//			returnStr += getMinSumMaxVariableDeclaration(cvc, queryBlock, havingClause, isTupleCalculation);
	...
	public static String getMinSumMaxVariableDeclaration(GenerateCVC1 cvc, QueryBlockDetails queryBlock, Node n) {
		...
		if(n.getLeft().getType().equalsIgnoreCase(Node.getAggrNodeType())){
			...
			if(n.getLeft().getAgg().getFunc().equalsIgnoreCase(AggregateFunction.getAggMAX())){
				...
				returnStr += consGen.getMaxConstraint(cvc, queryBlock, n,true);// n.getRight().toCVCString(10, queryBlock.getParamMap()), 
//				return returnStr + getCVCForHavingConstraintRepeated(cvc, queryBlock, n.getLeft(), totalRows,groupNumber);
				...
				returnStr += consGen.getMinConstraint(cvc, queryBlock, n,true);// n.getRight().toCVCString(10, queryBlock.getParamMap())
//				return returnStr + getCVCForHavingConstraintRepeated(cvc, queryBlock,  n.getLeft(), totalRows, groupNumber);
			}
				}
			...
			if(n.getRight().getAgg().getFunc().equalsIgnoreCase(AggregateFunction.getAggMAX())){
				...
				returnStr += consGen.getMaxConstraint(cvc, queryBlock, n,false);//n.getLeft().toCVCString(10, queryBlock.getParamMap()),
//				return returnStr + getCVCForHavingConstraintRepeated(cvc, queryBlock, n.getRight(), totalRows,   groupNumber);
				...
				returnStr += consGen.getMinConstraint(cvc, queryBlock, n,false);// n.getLeft().toCVCString(10, queryBlock.getParamMap()),
//				return returnStr + getCVCForHavingConstraintRepeated(cvc, queryBlock, n.getRight(), totalRows,  groupNumber);
			}
...
//					returnStr += consGen.getMinConstraint(queryBlock, n,true);// n.getRight().toCVCString(10, queryBlock.getParamMap())
				...
				{
					 String s =consGen.getAssertConstraint(getCVCForHavingConstraintRepeated(cvc, queryBlock, n.getLeft(), totalRows,   groupNumber),
							n.getOperator(),
					 ...
					 if(Configuration.getProperty("cntFlag").equalsIgnoreCase("true")) {
						 String constraints = "";
						 for(int i=1; i<=queryBlock.getFinalCount();i++) {
						   if(n.getLeft() != null)
							   constraints += "\t" + consGen.getConstraintsForValidCountUsingTable(n.getLeft().getTable(), n.getLeft().getTable().getTableName(),i, 0).toString() + "\n";
						   if(n.getRight().getColumn() != null) 
							   constraints += "\t" + consGen.getConstraintsForValidCountUsingTable(n.getRight().getTable(), n.getRight().getTable().getTableName(),i, 0).toString() + "\n";

						 }
							if(s.contains("assert")) {
								s = s.split("assert")[1].trim();
								s = s.substring(0,s.lastIndexOf(")")).trim();
							}
							
							s = "(assert (and "+ s + " " + constraints + "))";	
						}
					 return s;
				}
			}
