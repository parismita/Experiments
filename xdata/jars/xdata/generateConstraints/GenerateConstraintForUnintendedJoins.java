...
public class GenerateConstraintForUnintendedJoins {
	 ...
	 * @return
	 */
	public static boolean involvedInEquiJoins(QueryBlockDetails qbt, Object conj, String tableName1, String tableName2, String col1, String col2) {
		Vector<Vector<Node>> equivalenceClasses = new Vector<Vector<Node>>();
		Vector<Node> joinCondsForEquivalenceClasses = new Vector<Node>();

		if(conj instanceof ConjunctQueryStructure){
			equivalenceClasses = ((ConjunctQueryStructure)conj).getEquivalenceClasses() ;
			joinCondsForEquivalenceClasses = ((ConjunctQueryStructure)conj).getJoinCondsForEquivalenceClasses() ;
		}
		if(conj instanceof DisjunctQueryStructure){
			equivalenceClasses = ((DisjunctQueryStructure)conj).getEquivalenceClasses() ;
			joinCondsForEquivalenceClasses = ((DisjunctQueryStructure)conj).getJoinCondsForEquivalenceClasses() ;

		}
		/**for each equivalence class in this conjunct*/
		for( Vector<Node> ec: equivalenceClasses)
					...
					if( k!= l)/**if this is not same equivalence class*/
						...
						if(ec.get(k).getTableNameNo().equalsIgnoreCase(tableName1) &&
						   ...
						   ec.get(l).getColumn().getColumnName().equalsIgnoreCase(col2))	
								return true;
		for( Node ec: joinCondsForEquivalenceClasses)

			/**for each node in this equivalance class*/
			if(ec.getLeft().getTableNameNo().equalsIgnoreCase(tableName1) &&
				ec.getLeft().getColumn().getColumnName().equalsIgnoreCase(col1) &&
				ec.getRight().getTableNameNo().equalsIgnoreCase(tableName2) &&
				ec.getRight().getColumn().getColumnName().equalsIgnoreCase(col2))	
								return true;
		return false;
	}
	 ...
	 * @return
	 */
	//parismita - wee need exta columns with common type not name <- for missing join
	public static ArrayList< ArrayList<Node> > getExtraColumnsWithCommonName(GenerateCVC1 cvc,  HashMap<String, Table> tablesList, QueryBlockDetails qbt, Object conj) {
		...
		if(Configuration.getProperty("cntFlag").equalsIgnoreCase("true"))	
			size = tableNamesNo.size() - 1;
		else
			size = tableNamesNo.size();
