		...
		for(int i=0; i<projectedCols.size(); i++){
			...
			if(n!= null && n.getType() != null 
				...
				if(count>1){/** If more than one tuple in this relation*/						
					constraintString += constrGen.getAssertConstraint(cvc,c, offset, c, (offset+nextPos), " = ");
				...
				equi joins with other relation nodes*/
					...
					if(c1 != null){
						...
						constraintString += constrGen.getAssertConstraint(cvc,c1, offset, c1, (offset+nextPos), " = ");
					}
				}
			}
		}
