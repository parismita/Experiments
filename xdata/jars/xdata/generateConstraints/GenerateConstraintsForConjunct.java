...
/**
 * This method is used to get positive constraints for the given conjunct of the
 * query block
 * This class contains different methods to generate constraints by considering
 * only specific conditions of the given conjunct
 * 
 * @author mahesh
 *
 */
...
public class GenerateConstraintsForConjunct {
	...
	/**
	 * This method generates constraints by considering all the conditions of the
	 * given conjunct
	 * 
	 * @param cvc
	 * @param queryBlock
	 ...
	 * @throws Exception
	 */
	public static String getConstraintsForConjuct(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
		...
		/**
		 * Get the constraints for the selection conditions of the form A.x = Constant
		 * of this conjunct
		 */
		...
		for (int k = 0; k < selectionConds.size(); k++) {
			...
			int count = cvc.getNoOfTuples().get(tableNo) * queryBlock.getNoOfGroups();
			/** We should generate the constraints across all groups */
			for (int l = 1; l <= count; l++) {
				ConstraintObject constrnObj = new ConstraintObject();
				constrnObj.setLeftConstraint(constraintGen.genPositiveCondsForPred(queryBlock,
						selectionConds.get(k).getLeft(), (l + offset - 1)));
				constrnObj.setRightConstraint(constraintGen.genPositiveCondsForPred(queryBlock,
						selectionConds.get(k).getRight(), (l + offset - 1)));
				constrnObj.setOperator(selectionConds.get(k).getOperator());
				constrList.add(constrnObj);
			}
			// constraintString += "ASSERT " +
			// constraintGen.genPositiveCondsForPred(cvc,queryBlock,
			// selectionConds.get(k),l+offset-1)+";" +"\n";
		}
		...
		if (!isTempJoin) {
			for (int k = 0; k < allConds.size(); k++)
				constraintString += GenerateJoinPredicateConstraints.getConstraintsForNonEquiJoins(cvc, queryBlock,
						allConds, "") + "\n";
		} else {
			for (Node n : allConds) {
		...
		/**
		 * get the constraints for the conditions of the form A.x=cons where cons is a
		 * string constant
		 */
		...
		for (int k = 0; k < stringSelectionConds.size(); k++) {
			...
			;
			for (int l = 1; l <= count; l++) {
				// cvc.getStringConstraints().add( "ASSERT " +
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// stringSelectionConds.get(k),l+offset-1)+";" +"\n" );
				// }
				/*
				 * conObj.setLeftConstraint(constraintGen.genPositiveCondsForPred(queryBlock,
				 * stringSelectionConds.get(k),l+offset-1));
				 * constrList.add(conObj);
				 */

				/************* TEST CODE: POOJA **********/
				// cvc.getStringConstraints().add(constraintGen.genPositiveCondsForPred(queryBlock,
				// stringSelectionConds.get(k),l+offset-1));
				stringConstraints.add(
						constraintGen.genPositiveCondsForPred(queryBlock, stringSelectionConds.get(k), l + offset - 1));
				/****************************************/
			}
		}
		/*
		 ...
		 * constraintString +=
		 * constraintGen.generateANDConstraintsWithAssert(constrList); // "ASSERT " +
		 * constraintGen.genPositiveCondsForPred(cvc,queryBlock,
		 * selectionConds.get(k),l+offset-1)+";" +"\n";
		 * }
		 */
		...
		/*********** TEST CODE: POOJA *******/
		Vector<String> solvedStringConstraint = cvc.getStringSolver().solveConstraints(stringConstraints,
		...
		for (int k = 0; k < likeConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++) {
				// cvc.getStringConstraints().add( "ASSERT " +
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// likeConds.get(k),l+offset-1)+";\n" );
				// }
				conObj.setLeftConstraint(
						constraintGen.genPositiveCondsForPred(queryBlock, likeConds.get(k), l + offset - 1));

				constrList.add(conObj);
			}
		}

		// constraintString += constraintGen.generateANDConstraintsWithAssert(cvc,
		// constrList); // "ASSERT " +
		// constraintGen.genPositiveCondsForPred(cvc,queryBlock,
		// selectionConds.get(k),l+offset-1)+";" +"\n";
		if (constrList != null && constrList.size() > 0) {
			constraintString += "(assert " + constraintGen.generateANDConstraints(constrList) + " )\n";
			// System.out.println(constraintString+""); // TEMPCODE : Rahul Sharma : For
			// Debugging
		}
		...
		constraintString += GenerateConstraintsForWhereClauseSubQueryBlock
				.getConstraintsForWhereClauseSubQueryBlock(cvc, queryBlock, conjunct);
		...
		for (DisjunctQueryStructure disjunct : conjunct.disjuncts) {
			Constraints constraints = GenerateConstraintsForDisjunct.getConstraintsForDisjuct(cvc, queryBlock,
					disjunct);
	...
	public static Constraints getConstraintsInConjuct(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct) throws Exception {
		...
		for (int k = 0; k < selectionConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++) {
				// constraintString +=
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// selectionConds.get(k),l+offset-1) +" AND ";
				ConstraintObject constrnObj = new ConstraintObject();
				constrnObj.setLeftConstraint(constrGen.genPositiveCondsForPred(queryBlock,
						selectionConds.get(k).getLeft(), (l + offset - 1)));
				constrnObj.setRightConstraint(constrGen.genPositiveCondsForPred(queryBlock,
						selectionConds.get(k).getRight(), (l + offset - 1)));
				constrnObj.setOperator(selectionConds.get(k).getOperator());
				...
				if (Configuration.getProperty("cntFlag").equalsIgnoreCase("true")) {
					if (n.getType().equalsIgnoreCase(Node.getColRefType())) {
						...
						validConstraints = constrGen.getConstraintsForValidCount(cvc, tableName, (l + offset - 1), 0);
				...
				// (IntExpr) ctx.mkInt(offSet)), ctx.mkInt(0)));
			}
		...
		if (cvc.getqStructure().getCaseConditionMap() != null && !cvc.getqStructure().getCaseConditionMap().isEmpty()) {
			 ...
			 */
			//// constraintString +=
			//// (GenerateConstraintsForCaseConditions.getCaseConditionConstraintsForOriginalQuery(cvc,cvc.getOuterBlock()));
			// constraintString += " AND ";

			ConstraintObject constrnObj = new ConstraintObject();
			constrnObj.setLeftConstraint(GenerateConstraintsForCaseConditions
					.getCaseConditionConstraintsForOriginalQuery(cvc, cvc.getOuterBlock()));
			constrObjList.add(constrnObj);
		}
		...
		for (int k = 0; k < allConds.size(); k++) {
			...
			if (allConds != null && allConds.size() > 0
					&& allConds.get(k).getType().equalsIgnoreCase(Node.getBaoNodeType())) {
				tableNo = getTableNameNoForBAONode(allConds.get(k));
			} else if (allConds != null && allConds.size() > 0
					&& (allConds.get(k).getLeft() != null &&
							allConds.get(k).getLeft().getType().equalsIgnoreCase(Node.getBaoNodeType()))) {

				tableNo = getTableNameNoForBAONode(allConds.get(k).getLeft());
			} else if (allConds != null && allConds.size() > 0
					&& allConds.get(k).getRight() != null &&
					allConds.get(k).getRight().getType().equalsIgnoreCase(Node.getBaoNodeType())) {
				tableNo = getTableNameNoForBAONode(allConds.get(k).getRight());
			} else {
				tableNo = allConds.get(k).getLeft().getTableNameNo();
			}
			if (tableNo == null || tableNo.isEmpty()) {
				tableNo = allConds.get(k).getLeft().getTableNameNo();
			}
			...
			for (int l = 1; l <= count; l++) {
				...
				// selectionConds.get(k),l+offset-1) +" AND ";
				ConstraintObject constrnObj = new ConstraintObject();
				constrnObj.setLeftConstraint(
						constrGen.genPositiveCondsForPred(queryBlock, allConds.get(k).getLeft(), (l + offset - 1)));
				constrnObj.setRightConstraint(
						constrGen.genPositiveCondsForPred(queryBlock, allConds.get(k).getRight(), (l + offset - 1)));
				constrnObj.setOperator(allConds.get(k).getOperator());
				constrObjList.add(constrnObj);

				ConstraintObject constrnObjNull = new ConstraintObject();
				String validConstraints = "";
				/* Removing NUll enumerations */
				Node n = allConds.get(k).getLeft();
				if (Configuration.getProperty("cntFlag").equalsIgnoreCase("true")) {
					if (n.getType().equalsIgnoreCase(Node.getColRefType())) {
						Column col = n.getColumn();
		 ...
		 * nonEquiJoinConstraint.substring(7,nonEquiJoinConstraint.length()-2);
		 * 
		...
		// forms the string constraints - parismita - add var-var ones if needed
		...
		for (int k = 0; k < stringSelectionConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++) {
				...
				stringConstraint = constrGen.genPositiveCondsForPred(queryBlock, stringSelectionConds.get(k),
						l + offset - 1);
		...
		for (int k = 0; k < likeConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++) {
				// stringConstraint+=
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// likeConds.get(k),l+offset-1)+" AND ";
				stringConstraint = constrGen.genPositiveCondsForPred(queryBlock, likeConds.get(k), l + offset - 1);
				ConstraintObject constObj = new ConstraintObject();
				constObj.setLeftConstraint(stringConstraint);
				constrObjList.add(constObj);

				// changes made by sunanda for count
				String validConstraints = "";
				if (Configuration.getProperty("cntFlag").equalsIgnoreCase("true")) {
					if (likeConds.get(k).getLeft().getType().equalsIgnoreCase(Node.getColRefType())) {
						Column col = likeConds.get(k).getLeft().getColumn();
		 ...
		 * stringConstraint = stringConstraint.substring(0,
		 * stringConstraint.length()-5);
		 * }
		 * constraints.stringConstraints.add(stringConstraint);
		 */
		...
		for (DisjunctQueryStructure disjunct : conjunct.disjuncts) {
			constraints = Constraints.mergeConstraints(cvc, constraints,
					GenerateConstraintsForDisjunct.getConstraintsForDisjuct(cvc, queryBlock, disjunct));
	...
	/**
	 * This method generates constraints by considering all the conditions of the
	 * given conjunct, except Non equi-join conds
	 * 
	 * @param cvc
	 * @param queryBlock
	 * @return
	 * @throws Exception
	 */
	public static String getConstraintsForConjuctExceptNonEquiJoins(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct) throws Exception {
		...
		for (int k = 0; k < selectionConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++) {
				...
				constraintString += ConstraintGenerator.genPositiveCondsForPred(queryBlock, selectionConds.get(k),
						l + offset - 1) + "\n";
				// constraintString += "ASSERT " +
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// selectionConds.get(k),l+offset-1)+";" +"\n";
		...
		for (int k = 0; k < stringSelectionConds.size(); k++) {
			...
			;
			for (int l = 1; l <= count; l++)
				cvc.getStringConstraints().add(ConstraintGenerator.genPositiveCondsForPred(queryBlock,
						stringSelectionConds.get(k), l + offset - 1) + "\n");
			// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
			// stringSelectionConds.get(k),l+offset-1)+";" +"\n" );
		}
		...
		for (int k = 0; k < likeConds.size(); k++) {
			...
			;
			for (int l = 1; l <= count; l++)
				cvc.getStringConstraints()
						.add(ConstraintGenerator.genPositiveCondsForPred(queryBlock, likeConds.get(k), l + offset - 1)
								+ ";\n");
		}
	...
	/**
	 * This method generates constraints by considering all the conditions of the
	 * given conjunct, without including constraints for the selection conditions
	 * 
	 * @param cvc
	 * @param queryBlock
	 * @return
	 * @throws Exception
	 */
	public static String getConstraintsForConjuctExceptSelectionConds(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct) throws Exception {
		...
		for (int k = 0; k < stringSelectionConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++)
				cvc.getStringConstraints().add(ConstraintGenerator.genPositiveCondsForPred(queryBlock,
						stringSelectionConds.get(k), l + offset - 1) + ";" + "\n");
			// "ASSERT " + GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
			// stringSelectionConds.get(k),l+offset-1)+";" +"\n" );
		}
	...
	/**
	 * This method generates constraints by considering all the conditions of the
	 * given conjunct, without including constraints for string selection conditions
	 * 
	 * @param cvc
	 * @param queryBlock
	 * @return
	 * @throws Exception
	 */
	public static String getConstraintsForConjuctExceptStringSelectionConds(GenerateCVC1 cvc,
			QueryBlockDetails queryBlock, ConjunctQueryStructure conjunct) throws Exception {
	...
	/**
	 * This method generates constraints by considering all the conditions of the
	 * given conjunct, except like conditions
	 * 
	 * @param cvc
	 * @param queryBlock
	 * @return
	 * @throws Exception
	 */
	public static String getConstraintsForConjuctExceptLikeConds(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct) throws Exception {
	...
	/**
	 * This method generates constraints by considering all the conditions of the
	 * given conjunct, except where clause subquery constraints
	 * 
	 * @param cvc
	 * @param queryBlock
	 * @return
	 * @throws Exception
	 */
	public static String getConstraintsForConjuctExceptWhereClauseSubQueryBlock(GenerateCVC1 cvc,
			QueryBlockDetails queryBlock, ConjunctQueryStructure conjunct) throws Exception {
		...
		for (int k = 0; k < selectionConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++)
				constraintString += "(assert "
						+ ConstraintGenerator.genPositiveCondsForPred(queryBlock, selectionConds.get(k), l + offset - 1)
						+ " )\n";
			// "ASSERT " + GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
			// selectionConds.get(k),l+offset-1)+";" +"\n";
		}
	...
	public static String generateJoinConditionConstraintsForNotExists(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
		...
		for (Node n : joinConds) {
			...
			OrConstrList = GenerateJoinPredicateConstraints.genNegativeCondsEqClassForAllTuplePairs(cvc, queryBlock,
					right, left, rightTuples, leftTuples);
		}
		 ...
		 * constraintString.length()-3);
		 */

		return constraintString.trim();
	}

	public static String generateConstraintsNotExists(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct, String relation) throws Exception {
		...
		for (int k = 1; k <= cvc.getNoOfOutputTuples(relation); k++) {
			...
			if (!conjunct.joinCondsAllOther.isEmpty()) {
				...
				for (Node n : joinConds) {
					...
					if (node != null) {
						OrConstraints.add(GenerateJoinPredicateConstraints.genNegativeCondsEqClass(cvc, queryBlock,
								other, node, k));
					}
				}
			...
			for (int i = 0; i < negativeSelConds.size(); i++) {
				...
				OrConstraints.add(
						GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock, negativeSelConds.get(i), k));
			}
			...
			/** get negative conditions for these nodes */
			Vector<Node> negativeStringSelConds = GenerateCVCConstraintForNode
					.getNegativeConditions(stringSelectionConds);
			...
			for (int i = 0; i < negativeStringSelConds.size(); i++) {
				...
				OrStringConstraints
						.add(ConstraintGenerator.genPositiveCondsForPred(queryBlock, negativeStringSelConds.get(i), k)
								+ "\n");
				// OrStringConstraints.add( "ASSERT " +
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// negativeStringSelConds.get(i),k)+";" );
			}
			...
			for (int i = 0; i < likeConds.size(); i++) {
				...
				OrStringConstraints.add(
						ConstraintGenerator.genPositiveCondsForPred(queryBlock, negativeLikeConds.get(i), k) + "\n");
				// OrStringConstraints.add( "ASSERT " +
				// GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// negativeLikeConds.get(i), k)+";");
			}

			if (!OrStringConstraints.isEmpty()) {
				// Vector<String> tempVector = cvc.getStringSolver().solveOrConstraints( new
				// Vector<String>(OrStringConstraints), cvc.getResultsetColumns(),
				// cvc.getTableMap());
	...
	/**
	 * This method is used to get negative constraints for the given conjunct of the
	 * query block
	 * 
	 * @param cvc
	 * @param queryBlock
	 ...
	 * @throws Exception
	 */
	public static String generateNegativeConstraintsConjunct(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct) throws Exception {
		...
		/**
		 * FIXME: Mahesh..Ask Amol why the below code and why cant't we use
		 * getconstraintsforequijoins()
		 */
		if (!conjunct.getEquivalenceClasses().isEmpty()) {
			...
			for (int i = 0; i < equivalenceClasses.size(); i++) { /** For each equivalence class list */
				...
				for (int j = 0; j < ec.size(); j++) {
					...
					String CVCStr = ConstraintGenerator
					...
					/**
					 * S = set of elements in ec which have a foreign key relationship with R.a <use
					 * closure for this>
					 */
					...
					for (int k = 0; k < ec.size(); k++) {
						...
						/**
						 * TODO Maintain a datasructure for list of PK and FK so as to compare in one
						 * pass.
						 */
						for (int l = 0; l < cvc.getForeignKeys().size(); l++) {
							...
							if ((fk.getLeft().getTable() == tableEce && fk.getLeft().getColumn() == colEce)
									&& (fk.getRight().getTable() == tableNulled
											&& fk.getRight().getColumn() == colNulled)) {
								/**
					...
					/**
					 * Now, we have in S, the nulled column along with the foreign key columns in
					 * that equivalence class
					 * that reference the nulled column and are not nullable.
					 * But, if EC - S, contains a nullable column which references the nulled
					 * column,
					 * AND also contains some other column which may or may not be nullable,
					 * then we cannot assign NULL to the nullable FK column. We then need to nullify
					 * the nullable FK column
					 * along with the nulled column. If EC - S, just contains a single nullable
					 * column referencing the
					 * nulled column, then we can assign NULL to that column in order to nullify the
					 * nulled column.
					 */
					...
					if (P.size() == 1
						 ...
						 */
						OrConstraints
								.add(GenerateCVCConstraintForNode.cvcSetNull(cvc, P.get(0).getColumn(), "1", true));
					} else {
						/**
						 ...
						 * or whether it is nullable. Because, then we cannot assign NULL to any of the
						 * columns in P
						 * This is because, NULLs cannot be equated and hence the joins in P will not be
						 * propogated up.
						 */
					 ...
					 * //In this example, P will hold 2 values and reverse constraint is to be added
					 * for
					 * //(O_SECTION[1].3 /= O_TAKES[1].4) OR (O_TAKES[1].4 /= O_SECTION[1].3)
					 */

					if (P != null && P.size() == 1) {
						OrConstraints.add(GenerateJoinPredicateConstraints.genNegativeConds(cvc, queryBlock, colNulled,
								P.get(0)));
					} else if (P != null && P.size() > 1) {
						for (int k = 0; k < P.size(); k++) {
							OrConstraints.add(GenerateJoinPredicateConstraints.genNegativeConds(cvc, queryBlock,
									colNulled, P.get(k)));
						}
					}
				}
		...
		Vector<Node> allConds = conjunct.getJoinCondsAllOther();
		/** get constraint */
		String constraint = GenerateJoinPredicateConstraints.getNegativeConstraintsForNonEquiJoins(cvc, queryBlock,
				allConds);
		...
		for (int k = 0; k < negativeSelConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++)
				OrConstraints.add(
						ConstraintGenerator.genPositiveCondsForPred(queryBlock, negativeSelConds.get(k), l + offset - 1)
								+ "\n");
		}
		...
		for (int k = 0; k < negativeStringSelConds.size(); k++) {
			...
			;
			for (int l = 1; l <= count; l++)
				OrStringConstraints.add(ConstraintGenerator.genPositiveCondsForPred(queryBlock,
						negativeStringSelConds.get(k), l + offset - 1) + "\n");
		}
		...
		for (int k = 0; k < likeConds.size(); k++) {
			...
			for (int l = 1; l <= count; l++)
				OrStringConstraints.add(ConstraintGenerator.genPositiveCondsForPred(queryBlock,
						negativeLikeConds.get(k), l + offset - 1) + "\n");
		}
		...
		if (conjunct.getAllSubQueryConds() != null) {
			for (int i = 0; i < conjunct.getAllSubQueryConds().size(); i++) {
				 ...
				 */
				/**
				 * FIXME:If the given conjunct has NOT EXISTS conditions, then negative of that
				 * becomes positive
				 */
				...
				if (queryBlock.getWhereClauseSubQueries() != null
					...
					/** We need to get positive constraints for this sub query */
						...
						negativeConstraint = GenerateConstraintsForWhereClauseSubQueryBlock
								.getConstraintsForWhereSubQueryConnective(cvc, queryBlock, subQNegative);
					}
	...
	public static Constraints generateNegativeConstraintsForConjunct(GenerateCVC1 cvc, QueryBlockDetails queryBlock,
			ConjunctQueryStructure conjunct) throws Exception {
		Constraints constraints = new Constraints();
		String constraintString = "";
		...
		for (int k = 0; k < negativeSelConds.size(); k++) {
			...
			;
			for (int l = 1; l <= count; l++) {
				// constraint+=GenerateCVCConstraintForNode.genPositiveCondsForPred(queryBlock,
				// negativeSelConds.get(k),l+offset-1) + " AND ";

				ConstraintObject constrObj = new ConstraintObject();
				constrObj.setLeftConstraint(ConstraintGenerator.genPositiveCondsForPred(queryBlock,
						negativeSelConds.get(k), l + offset - 1));
	...
	public static String processOrConstraints(Vector<String> OrConstraints) {
		...
		for (String constraint : OrConstraints)
			if (constraint.length() != 0) {
				...
				// str +=
				// constraint;//constraint.trim().substring(6,constraint.trim().length()-1)+" OR
				// ";
			}
		// str = str.substring(0,str.length()-4);
		// str+=";";
	...
	/**
	 * Get the table name number from the BAO node. It contains expression with a
	 * column
	 * So traverse and find the table name number of the column on which expression
	 * is given
	 * 
	 * @param Node - BAONode
	 * @return
