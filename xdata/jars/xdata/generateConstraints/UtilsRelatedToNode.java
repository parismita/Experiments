...
public class UtilsRelatedToNode {
	...
	public static int getNoOfTuplesForThisNode(GenerateCVC1 cvc, QueryBlockDetails queryBlock, Node node) throws Exception{
		...
		if(node.getQueryType() == 1)
			...
			// return cvc.getNoOfTuples().get(node.getTableNameNo());
			// return cvc.getNoOfTuples().get(node.getTableNameNo());
			return cvc.getNoOfOutputTuples(node.getTable().getTableName()); // equi class all tuples count - sunanda
	...
	public static Vector<Node> getStringSelectionCondMutations(Node stringCond) throws Exception{
		...
		/********* For LIKE operator ********/
		// if(Configuration.getProperty("printSQL").equalsIgnoreCase("true")) 
		// 	scMutants.addAll(getLikeStringMutations(stringCond));
		// else if(!stringCond.getType().equals(Node.getBroNodeSubQType()) && op.contains("~")) 
		scMutants.addAll(getLikeStringMutations(stringCond));
	 ...
	 * parismita
	 */
	public static Vector<Node> getLikeStringMutations(Node likeCond) throws Exception{
		
		Vector<Node> likeMutants = new Vector<Node>();
		
		Node lcm = null;
		Node likelcmformut = null;
		
		lcm = likeCond.clone();
		lcm.setOperator("~");

		likelcmformut = likeCond.clone();
		likelcmformut.setOperator("~");
		...
		if(!likeCond.getType().equals(Node.getBroNodeSubQType()) && likeCond.getOperator().contains("~")){
			Vector<Node> patternMutants = getPatternStringMutations(likeCond);
			likeMutants.addAll(patternMutants);
		}else{
			Vector<Node> patternMutants = getPatternStringMutations(likelcmformut);
			likeMutants.addAll(patternMutants);
		}
	 ...
	 * parismita
	 */
	public static Vector<Node> getPatternStringMutations(Node likeCond) throws Exception{
		
		Vector<Node> likeMutants = new Vector<Node>();
		
		Node lcm = null;
		Vector<Node> patternMutants = new Vector<Node>();
		String pattern= likeCond.getRight().getStrConst();
		int l=pattern.length();
		
		for(int i=0;i<l;i++) {
			if(Configuration.getProperty("printSQL").equalsIgnoreCase("true")){
				if(pattern.charAt(i)=='%' || pattern.charAt(i)=='_' )
				{String newPattern=pattern.substring(0,i)+pattern.substring(i+1,l);
	
					lcm = likeCond.clone();
					lcm.getRight().setStrConst(newPattern);
					lcm.setIsMutant(true);
					patternMutants.add(lcm);
					
				newPattern=pattern.substring(0,i)+"__"+pattern.substring(i+1,l);
				...
				newPattern=pattern.substring(0,i)+"_"+pattern.substring(i+1,l);
					
					lcm = likeCond.clone();
					lcm.getRight().setStrConst(newPattern);
					lcm.setIsMutant(true);
					patternMutants.add(lcm);}
				else if(i==0&&!pattern.isEmpty()){
					String newPattern=pattern.substring(0,1)+"_"+pattern.substring(1,l);
					lcm = likeCond.clone();
					if(!likeCond.getOperator().contains("~")) lcm.setOperator("~");
					lcm.getRight().setStrConst(newPattern);
					lcm.setIsMutant(true);
					patternMutants.add(lcm);
					newPattern=pattern.substring(0,1)+"%"+pattern.substring(1,l);
	 ...
	 * @throws Exception
	 */
	public static Vector<Node> getWhereConectiveMutations(Node subQCond) throws Exception{
		
		Vector<Node> Mutants = new Vector<Node>();
		if(subQCond.getType().equals(Node.getBroNodeSubQType())){
			return getStringSelectionCondMutations(subQCond);
		}
		Node lcm = null;
		lcm = subQCond.clone();
		lcm.setType(Node.getExistsNodeType());
		Mutants.add(lcm);
		
		lcm = subQCond.clone();
		lcm.setType(Node.getNotExistsNodeType());
		Mutants.add(lcm);
	 ...
	 * @throws Exception
	 */
	public static Vector<Node> getUnintendedMutations(ArrayList<Node> extraCol, Object conjunct) throws Exception{
		
		Vector<Node> joinconds = new Vector<Node>();
		if(conjunct instanceof ConjunctQueryStructure){
			joinconds.addAll(((ConjunctQueryStructure)conjunct).joinCondsForEquivalenceClasses);
		}
		if(conjunct instanceof DisjunctQueryStructure){
			joinconds.addAll(((DisjunctQueryStructure)conjunct).joinCondsForEquivalenceClasses);
		}
		Vector<Node> Mutants = new Vector<Node>();
		for(int i=0; i < extraCol.size() - 1; i++){
			...
			if(n1.getColumn().getColumnName().equalsIgnoreCase("xdata_cnt")) continue;
			if(n2.getColumn().getColumnName().equalsIgnoreCase("xdata_cnt")) continue;	
			Node eqJoin = UtilsRelatedToNode.getJoinOfExtraCol(n1, n2, joinconds);
						
			Node newnode = new Node();
			if(eqJoin!=null)
				newnode = eqJoin.clone();
			newnode.setLeft(n1);
			newnode.setType(Node.getBroNodeType());
			newnode.setRight(n2);
			newnode.setOperator("/=");//set type etc
			newnode.setIsMutant(true);
			Mutants.add(newnode);
		}
	 ...
	 * @throws Exception
	 */
	public static Node getJoinOfExtraCol(Node n1, Node n2, Vector<Node> eqJoin) throws Exception{
		for(Node n: eqJoin){
			if(n.getLeft().getTableNameNo().equalsIgnoreCase(n1.getTableNameNo())
			&& n.getRight().getTableNameNo().equalsIgnoreCase(n2.getTableNameNo())) 
			return n;
			 
			if(n.getLeft().getTableNameNo().equalsIgnoreCase(n2.getTableNameNo())
			&& n.getRight().getTableNameNo().equalsIgnoreCase(n1.getTableNameNo())
			&& (n.getJoinType().equals("INNER JOIN")|| n.getJoinType().equals("FULL OUTER JOIN")))
			return n;
	 ...
	 * @throws Exception
	 */
	public static Vector<Node> getColumnReplacement(GenerateCVC1 cvc, Node candidate) throws Exception{
		
		Vector<Node> Mutants = new Vector<Node>();		
		for(String table: cvc.getOuterBlock().getBaseRelations()){
			...
			for(Table t: cvc.getTablesOfOriginalQuery()){
				...
				if(table.replaceAll("\\d", "").equalsIgnoreCase(t.getTableName())){ // added by sunanda for column replacement bug fix
					// replaceAll("\\d", "")
					requiredTable =t;
			...
			if(requiredTable == null) continue;
			int size = requiredTable.getNoOfColumn()-1;
			...
			for(int j = 0; j < size; j++){/**For each column of this table*/ // noOfColumn - 1 is done by Sunanda to ignore count column
				...
				if(col!=null&&candidate.getColumn()!=null&&col.getCvcDatatype() == candidate.getColumn().getCvcDatatype()){
					...
					if(!replacement.equals(candidate)){
						...
						if(Configuration.getProperty("printSQL").equalsIgnoreCase("true")){
							Node lcm = new Node();
							lcm = replacement.clone();
							Mutants.add(lcm);
						}
						else{
							Node lcm = new Node();
							lcm.setLeft(candidate);
							lcm.setRight(replacement);
							lcm.setOperator("/=");
							lcm.setType(Node.getBroNodeType());
							lcm.setIsMutant(true);
							lcm.level=-1;
							Mutants.add(lcm);
						}
