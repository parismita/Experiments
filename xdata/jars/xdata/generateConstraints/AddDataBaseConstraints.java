		...
		try{
			...
			for(int i=0; i < cvc.getResultsetTables().size(); i++){
			   ...
			   String pkconstFor2 = "";
				...
				//make assert for and constraints
...
//testcode ends
	...
	public static String getPrimaryKeyConstUsingNonEquiMethod(ArrayList<Column> primaryKeys, int noOfTuples) {
		...
		return constr;
		// throw new UnsupportedOperationException("Unimplemented method 'getPrimaryKeyConstUsingNonEquiMethod'");
	}
	...
	public static String generateConstraintsForForeignKeys(GenerateCVC1 cvc) throws Exception{
        //System.out.println("in fk constrain\n");
		...
		try{
			...
			for (int fg=(size-1); fg >= 0 ;fg--){
				//System.out.println("fg value :  " + fg);
...
//	}
	...
	public static String getFkConstraint(GenerateCVC1 cvc, ForeignKey foreignKey, String fkTableNameNo, int fkCount, int fOffset) {
		...
		// Old foreign key constraints commented by Sunanda
		...
		if (cvc.getConstraintSolver().equals("cvc3")) {
			fkConstraintNew = getSMTforForeignKey(cvc, foreignKey, fkCount, fkOffset, offset);
		} else {
		...
		//System.out.println(fkConstraintNew);
		...
		for (int j = 1; j <= fkCount; j++) {
			...
			for (int k = 0; k < fCol.size(); k++) {
				...
				if (fSingleCol.getCvcDatatype() != null) {
					...
					if (fSingleCol.isNullable()) {
						orConstraints.add(constraintGenerator.getIsNullConditionZ3(cvc,tableName1,fSingleCol, j + fkOffset -1 + ""));
					}
			...
			if (fCol.size()>0) {
				...
				for(int i=1 ; i<= table1Tuples ; i++) //this loops on tuples in child table
				{
					...
					if(usingCnt)
					{
							...
							// 	// temp1 += "\t ( ISNULL_"+fSingleCol.getColumnName()+ConstraintGenerator.smtMap(fSingleCol, findex).toString() + ") \n";
							// } else {
							// 	temp1 = "ISNULL_" + fSingleCol.getCvcDatatype();
							// 	FuncDecl funIsNull = ctxFunDeclHashMap.get(temp1);
							// 	// System.out.println(funIsNull.apply(ConstraintGenerator.smtMap(fSingleCol, findex)));
							// 	// temp1 += "\t ( ISNULL_"+fSingleCol.getCvcDatatype()+ConstraintGenerator.smtMap(fSingleCol, findex).toString() + ") \n";
							// }
					}
