...
public class GenerateCVCConstraintForNode {
	...
	public static String genNegativeCondsForPredAgg(GenerateCVC1 cvc, QueryBlockDetails queryBlock, Node pred, String nulledAliasName, String nulledTableName){
		...
		// if(Configuration.getProperty("isCVC3").equalsIgnoreCase("false")){
			ConstraintGenerator conGen = new ConstraintGenerator();
			int count1 = cvc.getNoOfOutputTuples(pred.getLeft().getTable().getTableName());
			int count2 = cvc.getNoOfOutputTuples(pred.getRight().getTable().getTableName());
			for(int i=1; i<=count1; i++){
				for(int j=1; j<=count2; j++){
					String tempStr = "";
					tempStr += "(not " + conGen.smtMapNode(pred, ""+count1, ""+count2) +" )";	
					constraintString += "(and "+ tempStr + conGen.getConstraintsForValidCountUsingTable(pred.getLeft().getTable(), pred.getLeft().getTable().getTableName(), count1, 0) 
					+ conGen.getConstraintsForValidCountUsingTable(pred.getRight().getTable(), pred.getRight().getTable().getTableName(), count2, 0) 
					+")\n" ;
				}
			}
			constraintString += "(or " + constraintString + " )";
		// }
	 ...
	 * @return
	 */
	public static String cvcSetNull(GenerateCVC1 cvc, Column c, String index, boolean nullOrNotNull){
		...
		if(foundNullVal){
			if(nullOrNotNull)
				...
				return consGen.getAssertNotNullValue(c,index,nullVal);
...
public static String primaryKeysSetNotNull(GenerateCVC1 cvc){
		...
		try{
			...
			for(int i=0; i < cvc.getResultsetTables().size(); i++){
				...
				ArrayList<Column> nonNullKeys = new ArrayList<Column>(table.getNonNullKey());
				...
				for (Column element : nonNullKeys) { 
		            if (!primaryKeys.contains(element)) { 
		            	primaryKeys.add(element); 
		            } 
		        } 
				...
				temp1 = "\n(assert (forall ((ipk"+p+" "+cvc.enumArrayIndex+")) \n";
				...
				for(String col : table.getColumns().keySet()){
					if( primaryKeys.toString().contains(col)){
						...
						Expr ipk0 = ConstraintGenerator.ctx.mkIntConst("ipk0");
						...
						if(Configuration.isEnumInt.equalsIgnoreCase("true")){
							...
							ipk0 = ConstraintGenerator.ctx.mkConst("ipk0",currentSort);
						}
						String tableName = pkeyColumn.getTableName();
						
//						Column cntCol = cvc.getTableMap().getTable(tableName.toUpperCase()).getColumn(cvc.getTableMap().getTable(tableName.toUpperCase()).getNoOfColumn()-1); // added by sunanda for count
//						BoolExpr andWithCount =  ConstraintGenerator.ctx.mkGt((ArithExpr)ConstraintGenerator.smtMap(cntCol,  ipk0), ConstraintGenerator.ctx.mkInt(0)); // added by sunanda for count
...
//							temp1 += "\t" + andWithCount.toString(); // added by sunanda for count
						p++;
					}
				}
