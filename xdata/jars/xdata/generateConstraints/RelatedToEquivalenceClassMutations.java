...
public class RelatedToEquivalenceClassMutations {
	...
	public static boolean getConstraintsForNulledColumns(GenerateCVC1 cvc,	QueryBlockDetails qbt, Vector<Node> ec, Node eceNulled)	throws Exception {
		...
		if(P.size() == 1 
			...
			int count = cvc.getNoOfOutputTuples(P.get(0).getTable().getTableName()); // equi class all tuples count - sunanda
			String tempStr = "";
			for(int k=1;k<=count;k++){
				tempStr += GenerateCVCConstraintForNode.cvcSetNull(cvc,P.get(0).getColumn(), k+offset-1+"", true) ;
				// cvc.getConstraints().add(GenerateCVCConstraintForNode.cvcSetNull(cvc,P.get(0).getColumn(), k+offset-1+"", true));
			}
			cvc.getConstraints().add(conGen.generateSMTOrConstraints(null, tempStr));
		}
