...
public class ExtraGroupByMutationsInOuterQueryBlock {
	...
	public static void generateDataForkillingExtraGroupByMutationsInOuterQueryBlock(GenerateCVC1 cvc) throws Exception {
		...
		try{
			...
			GenerateCommonConstraintsForQuery.generateNullandDBConstraints(cvc,false); // changes made by sunanda
	...
	public static void generateDataForkillingExtraGroupByMutationsInOuterQueryBlock2(GenerateCVC1 cvc) throws Exception {
		try{		
			...
			for(Column col: extraColumn){
				...
				String declareVariableMinMax = "";
				
				int loopVar = 1;
				if (cvc.outerBlock.getAggConstraints().size() != 0)
					loopVar = 2;
				for(int i = 0; i < loopVar; i++) { // why twice?
					...
					cvc.updateTotalNoOfOutputTuples(cvc.getOuterBlock(), 2); // works for simple group by query
					cvc.getOuterBlock().setNoOfGroups(2); // added by Sunanda
...
//
//								String constraint	= GenerateConstraintsForHavingClause.getHavingClauseConstraints(cvc, qb, qb.getAggConstraints().get(k), qb.getFinalCount(), j, false);
//								havingEachGroupConstraint += constraint;
//								havingEachGroupNegConstraint += constrGen.getNegatedConstraint(constraint);
//							}
						for(int k=0; k < qb.getAggConstraints().size();k++) {
							declareVariableMinMax = GenerateConstraintsForHavingClause.getMinSumMaxVariableDeclaration(cvc, qb, qb.getAggConstraints().get(k));
							for(int l=0; l< qb.getNoOfGroups(); l++)
								{
								String constraint=GenerateConstraintsForHavingClause.getHavingClauseConstraints(cvc, qb, qb.getAggConstraints().get(k), qb.getFinalCount(), l) ;
								havingEachGroupConstraint += constraint;
								havingEachGroupNegConstraint += constrGen.getNegatedConstraint(constraint);
								}
						}
					...
					if(i == 0){ // why?
						...
						cvc.getConstraints().add(declareVariableMinMax);
