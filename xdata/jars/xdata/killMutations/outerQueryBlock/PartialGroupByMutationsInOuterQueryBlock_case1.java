		...
		try{ 
			...
			for(Node tempgroupByNode : groupbyNodes){
				...
				for(int k=0; k < outer.getAggConstraints().size();k++) {
					cvc.getConstraints().add(GenerateConstraintsForHavingClause.getMinSumMaxVariableDeclaration(cvc, outer, outer.getAggConstraints().get(k)));
					for(int l=0; l< outer.getNoOfGroups(); l++)
						{
							cvc.getConstraints().add(GenerateConstraintsForHavingClause.getHavingClauseConstraints(cvc, outer, outer.getAggConstraints().get(k), outer.getFinalCount(), l) );
						}
				}
