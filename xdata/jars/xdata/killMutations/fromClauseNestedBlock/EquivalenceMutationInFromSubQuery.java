...
public class EquivalenceMutationInFromSubQuery {
	...
	private static Logger logger = Logger.getLogger(EquivalenceMutationInFromSubQuery.class.getName());
	/**
	...
	public static void generateDataForkillingEquivalenceClassMutationsInFromSubquery(GenerateCVC1 cvc) throws Exception{
		...
		for(QueryBlockDetails qbt: cvc.getOuterBlock().getFromClauseSubQueries()){
			...
			logger.log(Level.INFO,"GENERATE DATA FOR Killing equivalence clause Mutations Inside From clause subquery block: "+ qbt);
			logger.log(Level.INFO,"\n----------------------------------");
			...
			for(ConjunctQueryStructure conjunct: qbt.getConjunctsQs()){
				...
				for(int i=(equivalenceClassesOrig.size()-1); i >= 0; i--){	
					...
					qbt.setEquivalenceClassesKilled(ec); 					
