...
/**
 * This class generates data sets to kill string selection conditions (of the form A.x relOP const) class mutations in the from clause nested subquery block
 * @author mahesh
 *
 */
public class StringSelectionMutationsInFromSubquery {
	private static Logger logger = Logger.getLogger(StringSelectionMutationsInFromSubquery.class.getName());
	/**
	 * Generates data to kill string selection conditions mutations inside from clause nested subquery block
	 * @param cvc
	 */
public static void generateDataForkillingStringSelectionMutationsInFromSubquery(	GenerateCVC1 cvc) throws Exception{
		...
		/** Kill string selection clause mutations in each from clause nested block of this query*/
		for(QueryBlockDetails qbt: cvc.getOuterBlock().getFromClauseSubQueries()){
			
			logger.log(Level.INFO,"\n----------------------------------");
			logger.log(Level.INFO,"GENERATE DATA FOR Killing string SELECTION clause Mutations Inside From clause subquery block: "+ qbt);
			logger.log(Level.INFO,"----------------------------------\n");
			
			/**Kill the string selection clause mutations in each conjunct of this sub query query block*/
				...
				for(int i=0; i < stringSelectionConds.size(); i++){
					...
					for(int j=0; j<scMutants.size(); j++){
						...
						if(!( scMutants.get(j).getOperator().equalsIgnoreCase(sc.getOperator())) ){
							...
							cvc.setTypeOfMutation( TagDatasets.MutationType.STRING, TagDatasets.QueryBlock.FROM_SUBQUERY );
							...
							for(QueryBlockDetails qb: cvc.getOuterBlock().getFromClauseSubQueries()){
								if(!(qb.equals(qbt))){
									...
									cvc.getConstraints().add( QueryBlockDetails.TraverseNestedQueryBlock(cvc, qb) );
							...
							/** Generate postive constraints for all the conditions of this sub query block conjunct */
							/**Here the mutated condition is also included in the list of string selection conditions of the subquery block*/
							...
							for(int k=0; k < qbt.getAggConstraints().size();k++) {
								cvc.getConstraints().add(GenerateConstraintsForHavingClause.getMinSumMaxVariableDeclaration(cvc, qbt, qbt.getAggConstraints().get(k)));
