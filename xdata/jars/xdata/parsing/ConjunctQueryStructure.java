...
public class ConjunctQueryStructure implements Serializable{
	...
	public Vector<DisjunctQueryStructure> getDisjuncts(){
	...
	public ConjunctQueryStructure(ConjunctQueryStructure left,ConjunctQueryStructure right){
		...
		inClauseConds.addAll(left.getInClauseConds());
		inClauseConds.addAll(right.getInClauseConds());
	...
	public ConjunctQueryStructure(ORNode andNode){
		allConds=new Vector<Node>();
		selectionConds=new Vector<Node>();
		inClauseConds=new Vector<Node>();
		stringSelectionConds = new Vector<Node>();
		...
		disjuncts.add(new DisjunctQueryStructure(andNode));
	...
	public Vector<Vector<Node>> createEqClassForNEJConditions(){
		...
		Vector<Node> allJoinConds = new Vector<Node>();
		Vector<Vector<Node>> equivalenceClasessTemp = new Vector<Vector<Node>>();

		allJoinConds.addAll(joinCondsAllOther);
		allJoinConds.addAll(allConds);
		// allJoinConds.addAll(subQJC);
		Vector<Node> ec = new Vector<Node>();
		temp = new Node();
		Node temp1, temp2;
		...
		{
			Node joinNode = tempAllJoinConds1.get(i);
			if(joinNode!=null && joinNode.operator!=null && !joinNode.operator.contains("="))
				tempAllJoinConds.add(joinNode);
		}
		int count = 0;
		int flag = 0;
		while (tempAllJoinConds.size() > 0) {
			if(tempAllJoinConds.get(0).getRight().getType().equalsIgnoreCase(Node.getValType()) || tempAllJoinConds.get(0).getLeft().getType().equalsIgnoreCase(Node.getValType())){
				tempAllJoinConds.remove(0);
				continue;
			}
			temp = tempAllJoinConds.get(0).getLeft();
			...
			// tempAllJoinConds.remove(count);
			equivalenceClasessTemp.add(new Vector<Node>(ec));

			logger.log(Level.FINE,"Added EC: " + ec.toString());
			ec.removeAllElements();
		}
		removeDuplicates(equivalenceClasessTemp);
		for(DisjunctQueryStructure disjunct:disjuncts){
			equivalenceClasessTemp.addAll(disjunct.createEqClassForNEJConditions());
		}
		return equivalenceClasessTemp;
	}
	...
	public void seggregateSelectionConds(){
		...
		for(Node n:inClauseConds){
			if(isStringSelection(n,1) ){
				for(Node k: n.getRight().getComponentNodes()){
				String str=k.getStrConst();
				if(str!=null && !str.contains("'"))
					k.setStrConst("'"+str+"'");
				}
			}//datetime not managed - parismita
		}
	...
	public static boolean isStringSelection(Node n,int flag) {
		if(n.getLeft() != null && n.getLeft().getType().equals(Node.getColRefType())/* */){
			if(flag==1  && !n.getRight().getType().equals(Node.getValType())){
				if(n.getRight().getType().equals(Node.getCompositeNodeType()) && n.getRight().getComponentNodes().size()>0){
					for(Node k : n.getRight().getComponentNodes()){
						if(!k.getType().equals(Node.getValType())) return false;
					}
				}
				else
				return false;
			...
			if(n.getRight() != null && (n.getRight().getStrConst() != null||(n.getRight().getComponentNodes()!=null&&n.getRight().getComponentNodes().size()==0)) && 
	...
	public static void createConjuncts(parsing.QueryStructure qParser){
		if(qParser.orNode != null) {
			// qParser.conjuncts.add(new ConjunctQueryStructure(qParser.orNode));

			for(ANDNode andNode:qParser.orNode.andNodes){
				qParser.conjuncts.add(new ConjunctQueryStructure(andNode));
			}
			if(qParser.orNode.andNodes == null || qParser.orNode.andNodes.size() == 0 && qParser.orNode.leafNodes.size() == 1){
				qParser.conjuncts.add(new ConjunctQueryStructure(qParser.orNode.leafNodes));
			}
			else if(qParser.orNode.andNodes == null || qParser.orNode.andNodes.size() == 0){
				qParser.conjuncts.add(new ConjunctQueryStructure(qParser.orNode));

			}
			else if(!qParser.orNode.leafNodes.isEmpty())
				qParser.conjuncts.add(new ConjunctQueryStructure(qParser.orNode.leafNodes));
		}
