...
public class Node implements Cloneable, Serializable, NodeInterface{
	...
	private static String castNodeType ="CAST NODE";
	...
	public boolean orderByDirAsc = true; //false = decs
	public Boolean fromBelowLevelFClauseSQ = false;
	// added by sunanda for correlation
	public Integer level ;
	public String sqTableAliasName;
	...
	public ColDataType getCastType() {//parismita
		return casttype;
	}
	public void setCastType(ColDataType t) {
		this.casttype= t;
	}
	/**
	...
	public void setCastExpression(CastExpression cExpression) {
		this.castExpression = cExpression;
	}
	...
	public String toString(){
		...
		else if(this.getType()!=null&&this.getType().equalsIgnoreCase(Node.getCastNodeType())){
			return this.getCastExpression().toString();
		}
		...
		if(this.getType()!=null&&this.getType().equalsIgnoreCase(Node.getValType())){
			...
			if(this.getAgg()==null) return "";
			return (this.getAgg().getFunc() + "(" + this.getAgg().getAggExp().toString() + ")");
		}
		else if (this.getLeft()!=null&&this.getLhsRhs()!=null){
			return "(" + this.getLeft().toString()  + this.getOperator() + this.getLhsRhs().toString() + ")";
		}
	...
	public String toSMTString(int base, HashMap<String, String> paramMap){
		if(this.getType().equalsIgnoreCase(Node.getAggrNodeType())){
		...
		else if(this!=null&&this.getType().equalsIgnoreCase(Node.getValType())){ //this value has to be a number
			String s = paramMap.get(this.getStrConst());
			if(s!=null){
				if(s.contains("PARAM"))
					return paramMap.get(this.getStrConst());
			...
			if(this!=null&&this.getOperator().equalsIgnoreCase("<")){
				if(base == 16)
	...
	//end
	public Column getOrgiColumn(){
		return this.originalCol;
	}

	public void setOrgiColumn(Column col){
	...
	public boolean containsConstant(){
		...
		else if(this.getType().equalsIgnoreCase(Node.getCastNodeType())){//parismita
			return this.getLeft().containsConstant();
		}
	...
	public Vector<Column> getColumnsFromNode(){
		...
		else if(this.getType().equalsIgnoreCase(Node.getCastNodeType())){
			Vector<Column> ret1 = new Vector<Column>();
			ret1 = this.getLeft().getColumnsFromNode();
			return ret1;
		}
	...
	public static String getCastNodeType() {
		return castNodeType;
	}
	
	/**
	 * @param castNodeType the castNodeType to set
	 */
	public static void setCastNodeType(String castNodeType) {
		Node.castNodeType = castNodeType;
	}
