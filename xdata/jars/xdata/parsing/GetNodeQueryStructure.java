	...
	public static boolean getJoinNodesForEC(ConjunctQueryStructure con, Node temp) {
		if (temp.getType().equalsIgnoreCase(Node.getBroNodeType())
			...
			if (temp.getLeft()!=null && temp.getRight()!=null 
					...
					// commented by sunanda - 18 aug 2024
					// (temp.getJoinType()==null  
					// || temp.getJoinType().equals(JoinClauseInfo.innerJoin)
					// ) &&
				 !temp.getLeft().isCorrelated && !temp.getRight().isCorrelated && !temp.getLeft().getColumn().getColumnName().equalsIgnoreCase("XDATA_CNT")) // changes made by sunanda for count
			{
				// System.out.println(temp.getLeft().getColumn().getColumnName());
